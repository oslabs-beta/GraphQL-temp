...
db.queryUsername = async (username) => {
...
db.queryGraph = async (graphId) => {
  ...
  try {
    ...
    console.error(`Unable to find graph with ID ${graphId}`);
    throw new Error(err);
  }
};
...
db.queryGraphs = async (userId) => {
  ...
  try {
    ...
    console.error(`Unable to find graphs for user ${userId}`);
    throw new Error(err);
  }
};
...
db.createUser = async (username, email, hashWord, role) => {
  // ensure that username not already created
  if (db.queryUsername(username)) throw Error("username already taken.");
  // create new user
  ...
  try {
    ...
    console.error(`Unable to create user with username ${username}`);
    throw new Error(err);
  }
};
